			...
			<div class="panel panel-default">
				...
				<div class="panel-body">	 
					<?php
						...
						$query1 = $db->prepare("SELECT nama_slide,judul_slide,deskripsi,link_slide,gambar FROM slide WHERE id_slide=?");
						$query1->bind_param("i", $id_slide);
						$id_slide = $_GET['id'];
						$query1->execute();
						$result = $query1->get_result();
						...
						if ( isset($_POST['ubah']) ){
							...
							if ( empty($foto) ){
								$query = $db->prepare("UPDATE slide SET nama_slide=?, judul_slide=?, deskripsi=?, link_slide=? WHERE id_slide=?");
								$query->bind_param("ssssi", $judul,$judul_slide,$deskripsi,$url,$id_slide);
								$query->execute();
								echo '<div class="alert alert-success" role="alert" style="font-size : 12px; padding : 5px;">';
								echo '<button type="button" class="close" data-dismiss="alert">&times;</button>';
								...
								echo '</div>';
									...
									Uploadslide($unik,$tfoto);
									$query = $db->prepare("UPDATE slide SET nama_slide=?, judul_slide=?, deskripsi=?, link_slide=?, gambar=? WHERE id_slide=?");
									$query->bind_param("sssssi", $judul,$judul_slide,$deskripsi,$url,$unik,$id_slide);
									$query->execute();
